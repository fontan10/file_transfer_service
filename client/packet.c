                             ...
                             FILE *fptr)
{
    ...
    packet->size = size;
    packet->filename = (char *)malloc(256);
...
char *Packet_to_char_array(struct Packet *packet, int *arraySize)
{
    ...
    int fileNameSize = strlen(packet->filename) + 1; // must add 1 for the null terminator

    infoSize = totalFragDigits + 1 + fragNoDigits + 1 + sizeDigits + 1 + fileNameSize + 1; // extra 1's for :
    ...
    // memcpy(infoString + infoSize - 1, ":", 1);
    infoString[infoSize - 1] = ':';
